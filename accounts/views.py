    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                mail_subject='Por favor activa tu cuenta en Abastece'
                body= render_to_string('accounts/account_verification_email.html', {
                    'user': user,
                    'domain': current_site,
                    'uid': urlsafe_base64_encode(force_bytes(user.pk)), # transforma la pk en una coleccion de caracteres
                ...
                email1 =mail.EmailMessage(
                    mail_subject,
                    body,
                    'jeisson.vasquez@isa-america.com',
                    [to_email],
                ...
                #send_email.send()
                #print('aqui esta la conecxion')
                #print(send_email.connection)
...
def login(request):
    ...
    if request.method == 'POST':
        email = request.POST['email_correo']
        ...
        if user is not None:
            try:
                ...
                if is_cart_item_exists:
                    ...
                    # vaariaciones cuando el usuario no estaba en sesion
                    product_variation =[]
                    id = []
                    for item in cart_item:
                        variation = item.variations.all()
                        product_variation.append(list(variation))
                    # variaciones cuando el usuario estaba en sesion
...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            messages.success(request,'Un email te fue enviado para resetear tu password')
            return redirect('login')
        else:
...
def resetPassword(request):
    if request.method == 'POST':
        ...
        if password == confirm_password:
            ...
            user.save()
            messages.success(request,'Se reseteo correctamente')
            return redirect('login')
        else:
    ...
    if request.method == 'POST':
        ...
        if new_password == confirm_password :
            ...
            if success:
                ...
                messages.success(request,'Su password se ha cambiado correctamente')
